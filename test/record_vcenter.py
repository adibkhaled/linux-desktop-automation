...
# 
"""
The auto-test's all UI actions (click, scroll, input text, assert, wait) are used to recod the x and y coordinates.
"""
...
os.chdir("/home/vimec/v-center-testing")
    ...
    with mss.mss() as sct:
        ...
        if template is None:
            raise FileNotFoundError(
                "No image found to search screen for. {}".format(image)
            )
...
def on_click(x, y, button, pressed):
    ...
    if button == mouse.Button.left and not pressed:
        ...
        if top_release != top_press and left_release != left_press:
            image_count += 1
            print(
                "Drawn {} to {} and {} to {}".format(
                    top_press, top_release, left_press, left_release
                )
            )

            x, y, width, height = create_rect(
                left_press, top_press, left_release, top_release
            )
            ...
            with mss.mss() as sct:
                ...
                output = "base-{count}-{top}x{left}_{width}x{height}.png".format(
                    **monitor
                )
...
def play(input_file):
    ...
    for i in content:
        ...
        if "SEARCH" in i:
            status = loopScreen(i.strip().split(" ")[1], 2)
            if status == 0:
                verify += 1
